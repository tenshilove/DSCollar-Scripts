...
/|/ ---------- REVISED FROM OPENCOLLAR 6.5.0 ----------
/|/ This is where the leash script recognizes commands.
/|/ I am open to suggestions as to what commands to use. For now I will be using the following:
/|/ leash for leashing
/|/ unleash for unleashing
/|/ follow for follow mode
/|/ roam for ending follow mode
/|/ pull for yanking leash
/|/ heel for parking
/|/ move for releasing from parking
/|/ come for beckoning
/|/ post for posting
...
UserCommand(integer iAuth, string sMessage, key kMessageID, integer bFromMenu) {
    ...
    if (iAuth == CMD_EVERYONE) {
        ...
        if (sMessage == "leash menu" || sMessage == "menu leash" || sMessage == "leashmenu"){
            list lButtons;
            if (kMessageID != g_kWearer) lButtons += "Leash";/|/ Only if not the wearer.
            else lButtons += ["-"];
            ...
            if (kMessageID == g_kLeashedTo) lButtons += "Pull";/|/only if leash holder
            else lButtons += "-";
            lButtons += ["Follow"];
            lButtons += ["Post","Pass"];
            lButtons += ["Length"];
...
//program_version Firestorm-Release 5.0.1.52150 - Eiko Zeffirelli
//last_compiled 05/09/2017 08:30:43
//mono
